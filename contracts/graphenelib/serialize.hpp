...
#define GRAPHENELIB_REFLECT_MEMBER_OP( r, OP, elem ) \
  OP t.elem

/**
 *  @def GRAPHENE_SERIALIZE(TYPE,MEMBERS)
 *
...
#define GRAPHENE_SERIALIZE( TYPE,  MEMBERS ) \
 ...
 friend DataStream& operator << ( DataStream& ds, const TYPE& t ){ \
    return ds BOOST_PP_SEQ_FOR_EACH( GRAPHENELIB_REFLECT_MEMBER_OP, <<, MEMBERS );\
 }\
 template<typename DataStream> \
...
#define GRAPHENE_SERIALIZE_DERIVED( TYPE, BASE, MEMBERS ) \
 template<typename DataStream> \
 friend DataStream& operator << ( DataStream& ds, const TYPE& t ){ \
