...
#define N(X) graphene::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<graphene::chain::name> : private hash<uint64_t> {
      typedef graphene::chain::name argument_type;
...
FC_REFLECT( graphene::chain::name, (value) )
